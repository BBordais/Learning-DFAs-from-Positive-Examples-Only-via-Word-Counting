...
def simpledot2DFA(nameFile = None):
	import re
	if (nameFile is not None):
		...
		for line in s:
			...
			if 'doublecircle' in line:
				...
				final_states += [int(num[0])]
			
			if '->' in line and mode != 'transitions':
				init_state = line[-1]
				num = re.findall(r'\d+', line)
				init_state = int(num[-1])
				mode = 'transitions'
				continue
			
			if mode == 'transitions':
				num = re.findall(r'\d+', line)
				first_state = int(num[0])
				second_state = int(num[1])
				letter = line.split('"')[1]
				try:
					transitions[first_state][letter] = second_state
				except:
		...
		formula_dfa = DFA(init_state=init_state,final_states=final_states,transitions=transitions)
		return formula_dfa
	else:
		return None
